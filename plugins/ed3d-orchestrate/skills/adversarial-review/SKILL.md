...
The `adversary` dispatch uses preferred model `gpt-5.5` and effort `xhigh` on its first attempt, expressed as `model="gpt-5.5"` and `reasoning_effort="xhigh"` overrides. Invoke the named resource through Copilot's native agent/subagent delegation mechanism; do not call the Skill loader for agent names. Preserve the exact review prompt, plan path, `BASE_SHA`, `HEAD_SHA`, `NONCE`, prior issues, role, and working directory.
