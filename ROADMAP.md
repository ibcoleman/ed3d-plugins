...
## Landed: ed3d-orchestrate 0.4.3 - Astra adversary preference (2026-09-08)

- The operator selected `gpt-6-astra` with medium effort for adversary dispatches. The current Copilot CLI task-tool catalog lists this model and effort; other roles retain `gpt-5.6-luna` with high effort.
- The bounded pinned-first policy and Auto fallback are unchanged. Direct launches use CLI preferences; repository edits do not update installed plugin copies or existing `/subagents` settings.
- This catalog observation supersedes the dormant model-id follow-up below. It confirms advertised support, not successful dispatch on every account.
...
- **Model-id verification — superseded by 0.3.4; dormant under 0.5.0.** catalog verification remains dormant: no future catalog/API/operator verification event has occurred. This plan's prose pins, validator needles, and release do not count as verification. Reactivate catalog verification only after a future event confirms that explicit model selection is available again; next action is to record that event and re-check `gpt-5.6-astra`/`gpt-5.6-luna` against it before changing the dispatch policy.
...
- **Polytoken ↔ ed3d-plugins prompt cross-pollination** (audit complete 2026-09-01; implementation deferred). Portability audit: `docs/research/2026-09-01-polytoken-copilot-portability-audit.md`; reproducible evidence: `docs/research/2026-09-01-polytoken-copilot-portability-audit.evidence.json`. Finding: selective ports are worthwhile, while full facet/VFS/runtime parity is a large, low-fidelity effort. Recommended bounded follow-ups (not landed): (1) add the missing operator handoff/approval boundary between plan and builder in a Copilot plan-then-execute path (plan→review→build ordering already exists); (2) strengthen researcher/plan-reviewer/general-purpose output contracts and scoped grants; (3) defer facet parity, dynamic transclusion, and broad hook parity until a separate design. The audit records that the previously named AirPods case study is absent from this checkout and marks it unresolved.
